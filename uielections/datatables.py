# Modified from https://github.com/orf/datatables/ @ 5202d16
...
class DataTable(object):
    def __init__(self, params, model, query, columns, count_query=None):
        self.params = params
        self.model = model
        self.query = query
    ...
    def count(self, query, filtered=None):
        return query.count()
        # if self.count_query:
        #     return self.count_query.scalar()
        # else:
        #     return query.count()

    def order_by(self, query, column, direction):
        try:
            ...
            query = query.order_by(desc(column.model_name) if direction == "desc" else asc(column.model_name))
        else:
    ...
    def _json(self):
        ...
        for order in ordering.values():
            ...
            query = self.order_by(query, column, direction)
    ...
    def get_value(self, key, instance):
        ...
        if isinstance(instance, self.model):
            ...
            try:
                ...
                r = getattr(getattr(instance, self.model.__name__), attr)

        if key.filter is not None:
            r = key.filter(instance, r)
